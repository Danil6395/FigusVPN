...
//            }
    ...
    /**
     * Импорт серверов из строки (Base64 или список ссылок)
     */
    fun importBatchConfig(config: String): Int {
        try {
            // В твоей версии нужно 3 параметра: (строка, ID подписки, append: Boolean)
            // Мы ставим append = true, чтобы сервера добавлялись в список
            val result = AngConfigManager.importBatchConfig(config, subscriptionId, true)
            
            // result — это Pair<Int, Int>. Нам нужно первое число (успешные импорты)
