...
object V2rayConfigManager {
    ...
    private fun getV2rayCustomConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        // --- HARDCODED MTU FOR RUSSIA ---
        inboundTun.settings?.mtu = 1280
    ...
    private fun getInbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            ...
            if (needTun()) {
                ...
                // --- FORCE MTU 1350 FOR RUSSIA BYPASS ---
                inboundTun?.settings?.mtu = 1280
            ...
            when (config.policyGroupType) {
                "1" -> {
                    val balancer = V2rayConfig.RoutingBean.BalancerBean(
                        ...
                        strategy = V2rayConfig.RoutingBean.StrategyObject(type = "leastLoad")
                    )
                ...
                "2" -> {
                    val balancer = V2rayConfig.RoutingBean.BalancerBean(
                        tag = AppConfig.TAG_BALANCER,
                        selector = lstSelector,
                        strategy = V2rayConfig.RoutingBean.StrategyObject(type = "random")
                    )
                    v2rayConfig.routing.balancers = listOf(balancer)
                }
                "3" -> {
                    val balancer = V2rayConfig.RoutingBean.BalancerBean(
                        tag = AppConfig.TAG_BALANCER,
                        selector = lstSelector,
                        strategy = V2rayConfig.RoutingBean.StrategyObject(type = "roundRobin")
                    )
                    v2rayConfig.routing.balancers = listOf(balancer)
                }
                else -> {
                    val balancer = V2rayConfig.RoutingBean.BalancerBean(
                        tag = AppConfig.TAG_BALANCER,
                        selector = lstSelector,
                        strategy = V2rayConfig.RoutingBean.StrategyObject(type = "leastPing")
                    )
                    v2rayConfig.routing.balancers = listOf(balancer)
    ...
    private fun updateOutboundFragment(v2rayConfig: V2rayConfig): Boolean {
        try {
            // --- FORCE ENABLE FRAGMENT FOR RUSSIA (BYPASS DPI) ---
            // Removed check for PREF_FRAGMENT_ENABLED to enable it by default
            ...
            // --- OPTIMIZED SETTINGS FOR RUSSIA ---
            val packets = "tlshello"
            val length = "10-20"
            val interval = "10-20"
